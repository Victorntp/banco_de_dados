...
-- 10. Liste o valor total dos pedidos por cliente, incluindo pedidos feitos por clientes que foram 
-- excluídos (nome aparecerá como NULL
select clientes.id, clientes.nome, sum(pedidos.valor) as valor_total from pedidos
...
select pedido_id as pedido, max(valor) as 'valor pedido' from itens_pedido group by pedido_id;


-- 12. Qual o valor do pedido mais barato registrado?
select pedido_id as pedido, min(valor) as 'valor pedido' from itens_pedido group by pedido_id;

-- 14. Qual o pedido mais caro, exibir o nome do cliente e o valor do pedido.

select nome, max(valor) as 'maior valor' from pedidos
left join clientes on clientes.id = pedidos.cliente_id;
...
-- 22. Liste os 3 clientes que mais gastou, exibir nome do cliente e o valor gasto.Esses exercícios a você a praticar e compreender como utilizar essas funções e junções no dia a dia, tanto para análise de dados quanto para consultas complexas no banco de dados
...
-- 2
select count(valor) from pedidos;
